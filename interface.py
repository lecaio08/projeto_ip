...
class UI:
    ...
    def draw_hud(self, player, coins, start_time):
        ...
        color_time  = RED if remaining < 10 else WHITE
        self.draw_text(f"Tempo: {remaining:.1f}", 20, color_time, 20, 10, align='left')
        self.draw_text(f"Vidas: {player.lives}", 24, WHITE, WIDTH/2, 10, align='center')
        right_x     = WIDTH - 20        
        color_apple = GREEN if player.apples > 0 else WHITE
        self.draw_text(f"Maçã: {player.apples}/3", 18, color_apple, right_x, 10, align='right')
        
        if player.lives < 3 and player.apples > 0:
             self.draw_text("[A] Curar!", 14, GREEN, right_x, 28, align='right')
        
        martelo_status = 1 if player.has_hammer else 0
        ...
        self.draw_text(f"Moedas: {coins}", 18, GOLD, right_x, 70, align='right')

    def screen_start(self):
        self.screen.fill(BLACK)
        self.draw_text("Donkey Kong", 48, YELLOW, WIDTH/2, HEIGHT/4)
        self.draw_text("Aperte [ENTER] para jogar! | [O]: Opções", 22, WHITE, WIDTH/2, HEIGHT/2)
        pygame.display.flip()
        return self._wait_key(['RETURN', 'o'])
